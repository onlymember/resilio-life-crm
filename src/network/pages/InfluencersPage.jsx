...
import { dbGetInfluencers, dbGetGeography } from '../../lib/database.js'
import { useNavigate } from 'react-router-dom'

const PAGE_SIZE = 30

export default function InfluencersPage({ onOpenCreate }) {
  ...
  const load = useCallback(async (pg = 0, s = search, f = filters) => {
    setLoading(true)
    try {
      const res = await dbGetInfluencers({ page: pg, pageSize: PAGE_SIZE, search: s || undefined, ...f })
      if (pg === 0) setRows(res.rows); else setRows(p => [...p, ...res.rows])
      setTotal(res.total); setPage(pg)
    } catch(e) {
      console.error('InfluencersPage load:', e.message)
    ...
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          style={{ width:'100%', paddingLeft:34, paddingRight:12, paddingTop:10, paddingBottom:10, background:'rgba(139,92,246,0.07)', border:'1px solid rgba(139,92,246,0.2)', borderRadius:10, color:'var(--text-primary)', fontSize:13, outline:'none' }}
        />
      </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          ...
          {hasMore && (
            <button
              onClick={() => load(page + 1)}
              disabled={loading}
              style={{ padding:'12px', borderRadius:10, background:'rgba(139,92,246,0.08)', border:'1px solid var(--border-violet)', color:'var(--text-secondary)', cursor:'pointer', fontSize:13 }}
            >
