...
import { Palette, Plus, Edit3, Trash2, X, Save, Users, DollarSign, Clock, CheckCircle, Circle, AlertCircle, Search, Globe, Briefcase } from 'lucide-react'

const DEMO_EQUIPO = [
  { id:'e1', nombre:'Lucía Fernández', rol:'Directora Creativa', pais:'Argentina', ciudad:'Buenos Aires', especialidad:'Branding & Identidad', portfolio:'luciadesign.com', contacto:'lucia@resilio.com', instagram:'@lucia_design', estado:'activo', proyectos:['Rebrand Elevare','Web Resilio'] },
  { id:'e2', nombre:'Carlos Vega', rol:'Motion Designer', pais:'México', ciudad:'Ciudad de México', especialidad:'Motion Graphics & 3D', portfolio:'carlosvega.mx', contacto:'carlos@resilio.com', instagram:'@carlosvega_mx', estado:'activo', proyectos:['Videos Campaigns'] },
  { id:'e3', nombre:'Sophie Müller', rol:'UX Designer', pais:'Alemania', ciudad:'Berlín', especialidad:'UX/UI & Producto', portfolio:'sophiemüller.de', contacto:'sophie@resilio.com', instagram:'@sophie_ux', estado:'activo', proyectos:['App Resilio','Dashboard'] },
  { id:'e4', nombre:'André Costa', rol:'Fotógrafo', pais:'Brasil', ciudad:'São Paulo', especialidad:'Fotografía de Marca', portfolio:'andrecosta.br', contacto:'andre@resilio.com', instagram:'@andrecosta_photo', estado:'activo', proyectos:['Sesión Elevare'] },
  { id:'e5', nombre:'María López', rol:'Copywriter', pais:'España', ciudad:'Madrid', especialidad:'Copy & Storytelling', portfolio:'marialopez.es', contacto:'maria@resilio.com', instagram:'@maria_copy', estado:'freelance', proyectos:[] },
]

const EQUIPO_EMPTY = { nombre:'', rol:'', pais:'', ciudad:'', especialidad:'', portfolio:'', contacto:'', instagram:'', estado:'activo', proyectos:[] }
...
function EquipoTab() {
  ...
  const [form, setForm] = useState(EQUIPO_EMPTY)
  ...
  const save = () => {
    ...
    if (form.id) setEquipo(p=>p.map(e=>e.id===form.id?form:e))
    else setEquipo(p=>[...p,{...form,id:`e${Date.now()}`}])
    setModal(null)
  }
  ...
  return (
    <div style={{animation:'fadeIn 0.3s ease'}}>
      ...
      <div style={{display:'flex',gap:10,marginBottom:20,flexWrap:'wrap'}}>
        ...
        <button className="btn btn-primary" onClick={()=>{setForm(EQUIPO_EMPTY);setModal('form')}}><Plus size={15}/>Agregar Miembro</button>
      </div>
          ...
          <div key={m.id} className="card" style={{padding:'20px'}}>
            <div style={{display:'flex',alignItems:'flex-start',gap:12,marginBottom:14}}>
              ...
              <div style={{display:'flex',gap:4,flexShrink:0}}>
                <button onClick={()=>{setForm(m);setModal('form')}} style={{padding:'4px 7px',borderRadius:7,color:'var(--primary-violet-light)',background:'rgba(139,92,246,0.1)',border:'none',cursor:'pointer'}}><Edit3 size={11}/></button>
                <button onClick={()=>setEquipo(p=>p.filter(e=>e.id!==m.id))} style={{padding:'4px 7px',borderRadius:7,color:'#F87171',background:'rgba(239,68,68,0.1)',border:'none',cursor:'pointer'}}><Trash2 size={11}/></button>
              </div>
            </div>
            ...
            {m.proyectos.length>0&&(
              <div>
                <div style={{fontSize:10,color:'var(--text-secondary)',marginBottom:4}}>Proyectos asignados</div>
                <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                  {m.proyectos.map(p=><span key={p} style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:'rgba(139,92,246,0.1)',color:'var(--primary-violet-light)',border:'1px solid var(--border-violet)'}}>{p}</span>)}
                </div>
              </div>
            )}
            {m.portfolio&&<a href={`https://${m.portfolio}`} target="_blank" rel="noreferrer" style={{display:'block',marginTop:10,fontSize:11,color:'var(--primary-violet-light)',textDecoration:'underline'}}>🔗 {m.portfolio}</a>}
          </div>
        ))}
          ...
          <div style={{width:'100%',maxWidth:520,background:'var(--bg-secondary)',border:'1px solid var(--border-violet)',borderRadius:20,maxHeight:'90vh',overflowY:'auto',animation:'fadeIn 0.3s ease'}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{padding:'20px 24px',display:'flex',flexDirection:'column',gap:12}}>
              {[
                ...
                <div key={item.f}><label style={{fontSize:12,fontWeight:500,color:'var(--text-secondary)',display:'block',marginBottom:6}}>{item.l}</label><input className="input-field" value={form[item.f]} onChange={e=>set(item.f,e.target.value)} placeholder={item.ph}/></div>
              ))}
              <div>
                ...
                <div style={{display:'flex',gap:8}}>
                  {['activo','freelance','inactivo'].map(s=>(
                    <button key={s} onClick={()=>set('estado',s)} style={{flex:1,padding:'8px',borderRadius:10,fontSize:12,fontWeight:600,border:`1px solid ${form.estado===s?(estadoColor[s]||'#9CA3AF'):'var(--border-violet)'}`,background:form.estado===s?`${estadoColor[s]||'#9CA3AF'}22`:'rgba(139,92,246,0.05)',color:form.estado===s?(estadoColor[s]||'#9CA3AF'):'var(--text-secondary)',transition:'all 0.2s',textTransform:'capitalize',cursor:'pointer'}}>{s}</button>
                  ))}
                </div>
              </div>
            </div>
...
const STATUS_CFG = {
  idea:        { label: 'Idea',        color: '#A78BFA', bg: 'rgba(167,139,250,0.15)', col: 0 },
  proposal:    { label: 'Propuesta',   color: '#FCD34D', bg: 'rgba(252,211,77,0.15)',  col: 1 },
  in_progress: { label: 'En proceso',  color: '#8B5CF6', bg: 'rgba(139,92,246,0.15)', col: 2 },
  completed:   { label: 'Completado',  color: '#4ADE80', bg: 'rgba(74,222,128,0.15)', col: 3 },
  cancelled:   { label: 'Cancelado',   color: '#F87171', bg: 'rgba(239,68,68,0.15)',  col: 4 },
  archivo:     { label: 'Archivo',     color: '#9CA3AF', bg: 'rgba(156,163,175,0.15)', col: 5 },
}
const KANBAN_COLS = [
...
// ── Project Modal ────────────────────────────
const PROJ_EMPTY = { type:'monthly_retainer',clientId:'',name:'',description:'',status:'proposal',budget:0,budgetSpent:0,startDate:'',endDate:'',hoursTracked:0,hoursEstimated:0,deliverables:[],team:[] }
    ...
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',backdropFilter:'blur(8px)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20 }} onClick={onClose}>
      <div style={{ width:'100%',maxWidth:620,background:'var(--bg-secondary)',border:'1px solid var(--border-violet)',borderRadius:20,maxHeight:'92vh',overflowY:'auto',boxShadow:'var(--glow-violet),0 40px 80px rgba(0,0,0,0.5)',animation:'fadeIn 0.3s ease' }} onClick={e=>e.stopPropagation()}>
        <div style={{ padding:'24px 28px',borderBottom:'1px solid var(--border-violet)',display:'flex',alignItems:'center',gap:12 }}>
          ...
          <div><h2 style={{ fontSize:16,fontWeight:700 }}>{project?'Editar Proyecto':'Nuevo Proyecto'}</h2></div>
...
// ── Project Mini Card (for Kanban) ───────────
const ProjectKanbanCard = ({ project, client, onEdit, onDelete }) => {
  ...
  const hoursPct  = project.hoursEstimated > 0 ? Math.round(project.hoursTracked/project.hoursEstimated*100) : 0
  const completedDels = (project.deliverables||[]).filter(d=>d.status==='completed').length
  const totalDels = (project.deliverables||[]).length

  return (
    <div style={{ background:'var(--glass-bg)',border:'1px solid var(--border-violet)',borderRadius:12,padding:'14px 16px',marginBottom:10,transition:'all 0.2s',cursor:'default' }}
    ...
    >
      <div style={{ display:'flex',alignItems:'flex-start',gap:8,marginBottom:10 }}>
        ...
        <div style={{ display:'flex',gap:4,marginLeft:'auto',flexShrink:0 }}>
          <button onClick={onEdit} style={{ padding:'3px 6px',borderRadius:6,color:'var(--primary-violet-light)',background:'rgba(139,92,246,0.1)',border:'none',cursor:'pointer',display:'flex',alignItems:'center' }}><Edit3 size={10}/></button>
          <button onClick={onDelete} style={{ padding:'3px 6px',borderRadius:6,color:'#F87171',background:'rgba(239,68,68,0.1)',border:'none',cursor:'pointer',display:'flex',alignItems:'center' }}><Trash2 size={10}/></button>
        </div>
      </div>
      <div style={{ fontSize:13,fontWeight:600,marginBottom:4,lineHeight:1.3 }}>{project.name}</div>
      {client && <div style={{ fontSize:10,color:'var(--text-secondary)',marginBottom:10 }}>{client.name}</div>}
      <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
        ...
        {project.hoursEstimated > 0 && <ProgressBar label={`${project.hoursTracked}h / ${project.hoursEstimated}h`} value={project.hoursTracked} max={project.hoursEstimated} color="#A78BFA"/>}
      </div>
      {totalDels > 0 && (
        <div style={{ marginTop:8,fontSize:10,color:'var(--text-secondary)' }}>
          <CheckCircle size={10} style={{ display:'inline',marginRight:4,verticalAlign:'middle',color:'#4ADE80' }}/>
          {completedDels}/{totalDels} entregables
        </div>
      )}
    </div>
  )
...
// ── Client Modal ─────────────────────────────
const CLIENT_EMPTY = { name:'',contactPerson:'',email:'',phone:'',type:'retainer',status:'active',tipoCliente:'fijo' }
const TIPO_CLI_CFG = { fijo:{l:'Fijo',c:'#4ADE80'}, estandar:{l:'Estándar',c:'#60A5FA'}, especial:{l:'Especial',c:'#E879F9'} }
...
export default function CreativeAgencyView({ projects, clients, onSaveProject, onDeleteProject, onSaveClient, onDeleteClient, defaultTab = 'kanban' }) {
  ...
  return (
    <div style={{ padding:24,animation:'fadeIn 0.3s ease' }}>
            ...
            return (
              <div key={col.id}>
                ...
                <div style={{ minHeight:100 }}>
                  {colProjects.map(p => (
                    <ProjectKanbanCard key={p.id} project={p} client={clients.find(c=>c.id===p.clientId)}
                      onEdit={()=>setModal(p)} onDelete={()=>onDeleteProject(p.id)}
                    />
                  ))}
                  {colProjects.length === 0 && (
                    <div style={{ border:'1px dashed var(--border-violet)',borderRadius:10,padding:'20px',textAlign:'center',color:'var(--text-secondary)',fontSize:12 }}>Sin proyectos</div>
                  )}
                </div>
              </div>
            )
      ...
      {tab === 'clients' && (
        <div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:14 }}>
            {clients.filter(c => clientFilter==='all' || c.tipoCliente===clientFilter).map(c => {
              ...
              return (
                <div key={c.id} style={{ background:'var(--glass-bg)',border:'1px solid var(--border-violet)',borderRadius:14,padding:'18px 20px',transition:'all 0.2s' }}
      ...
      {modal  && <ProjectModal project={modal==='create'?null:modal} clients={clients} onSave={d=>{onSaveProject(d);setModal(null)}} onClose={()=>setModal(null)}/>}
